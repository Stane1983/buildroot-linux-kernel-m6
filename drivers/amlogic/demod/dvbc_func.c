...
static void dvbc_reg_initial(struct aml_demod_sta *demod_sta)
{
    ...
    printk("in dvbc_func, clk_freq is %d, adc_freq is %d\n", clk_freq, adc_freq);
    // disable irq
    apb_write_reg(0, 0xd0, 0);

    // reset
    //dvbc_reset();
    apb_write_reg(0, 0x4, apb_read_reg(0,0x4) &~ (1 << 4)); // disable fsm_en
	  apb_write_reg(0, 0x4, apb_read_reg(0,0x4) &~ (1 << 0)); // Sw disable demod
	  apb_write_reg(0, 0x4, apb_read_reg(0,0x4) | (1 << 0)); // Sw enable demod
	...
	apb_write_reg(0, 0x054, 0x2336043a);  // EQ_FIR_CTL, by raymond
	apb_write_reg(0, 0x068, 0x00c000c0);  // EQ_CRTH_SNR
	//apb_write_reg(0, 0x074, 0x5000260);  // EQ_TH_LMS  40db  19db
    ...
    //apb_write_reg(0, 0x020, 0x21353e54);  // PHS_reset & TIM_CTRO_ACCURATE  sw_tim_select=0
	  //apb_write_reg(0, 0x020, 0x21b53e54); //modified by qiancheng
	  apb_write_reg(0, 0x020, 0x61b53e54); //modified by qiancheng by raymond
    ...
    {
       max_frq_off = (1<<29) / symb_rate;   // max_frq_off = (400KHz * 2^29) / (AD=25 * symbol_rate=68750)
       tmp = 40000000 / adc_freq;
       max_frq_off = tmp * max_frq_off;
    }
    apb_write_reg(0, 0x02c, max_frq_off&0x3fffffff);  // max frequency offset, by raymond
    
    //apb_write_reg(0, 0x030, 0x011bf400);  // TIM_CTL0 start speed is 0,  when know symbol rate  
	  apb_write_reg(0, 0x030, 0x245cf451); //MODIFIED BY QIANCHENG
...
/************* hw state machine config **********/ 		
    ...
    //apb_write_reg(0, 0x0c0, 0xffffff68); // threshold
    //apb_write_reg(0, 0x0c0, 0xffffff68); // threshold
    //apb_write_reg(0, 0x0c0, 0xffff2f67); // threshold for skyworth
    apb_write_reg(0, 0x0c0, 0x061f2f67); // by raymond
/************* hw state machine config **********/ 		
 		
    apb_write_reg(0, 0x04c, 0x00008800);  // reserved

    //apb_write_reg(0, 0x050, 0x00000002);  // EQ_CTL0
    apb_write_reg(0, 0x050, 0x01472002);  // EQ_CTL0 by raymond
    
    //apb_write_reg(0, 0x058, 0xff550e1e);  // EQ_FIR_INITPOS
    apb_write_reg(0, 0x058, 0xff100e1e);  // EQ_FIR_INITPOS for skyworth
    ...
    apb_write_reg(0, 0x064, 0x010a1128);  // EQ_CRTH_TIMES for skyworth
    ...
    //apb_write_reg(0, 0x0a0, 0x0e028700);  // AGC_RFGAIN_CTRL 0x0e020800 now
    //apb_write_reg(0, 0x0a0, 0x0e03cd00);  // AGC_RFGAIN_CTRL 0x0e020800
    apb_write_reg(0, 0x0a0, 0x0e03cd11);  // AGC_RFGAIN_CTRL 0x0e020800 by raymond, if Adjcent channel test, maybe it need change.
    
    apb_write_reg(0, 0x004, apb_read_reg(0, 0x004)|0x33);  // IMQ, QAM Enable
    
    // start hardware machine
    //dvbc_sw_reset(0x004, 4); 
    apb_write_reg(0, 0x4, apb_read_reg(0,0x4) | (1 << 4));
		...
		struct aml_demod_dvbc *demod_dvbc)
{
   ...
   printk("f=%d, s=%d, q=%d\n", demod_dvbc->ch_freq, demod_dvbc->symb_rate, demod_dvbc->mode);
    ...
    // Set Tuner
    if(symb_rate == 6876) // don't configure tuner
        demod_sta->symb_rate = 6875;
    else
