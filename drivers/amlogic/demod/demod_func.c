...
void CPU_wait(int num)
{
	while(num --){;}
}
void gpio_reset(void)
{
  CLEAR_CBUS_REG_MASK(PERIPHS_PIN_MUX_8,0xf << 12);//


  CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_EN_N, (1 << IO_CSB ) ); 
  CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_EN_N, (1 << IO_SCK ) ); 
  CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_EN_N, (1 << IO_MOSI) ); 
  SET_CBUS_REG_MASK(PREG_PAD_GPIO3_EN_N, (1 << IO_MISO) ); 
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// SPI : interface
// IO_CSB ---\_____________________________________________________________________________________________/------------
// IO_SCK _______/-\__/-\__/-\__/-\_/-\___/-\__/-\__/-\__/-\__/-\_/-\___/-\__/-\__/-\__/-\__/-\_/-\___/-\_______________
// IO_MOSI_____|===X====X====X====X===X=====X==(command 16bits)===(data In 32bits)==x====x====x===x====_________________
// IO_MISO________________________________________________________(data Out32bits)==x====x====x===x====_________________
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int apb_read_reg_serial_dvbt(int addr)
{
	  int ck0;
	  int dataOut;
	  gpio_reset();
	  SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_CSB);
	  CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);
	  CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_MOSI);
  
	  dataOut = 0;	  
	  CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_CSB);
	  // Command 0000 == read
	  CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<< IO_MOSI);
	  SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);
	  SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);
	  SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);
	  SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);
	  
	  // Command addr
	  for (ck0=11;ck0>=0;ck0--)
	  {
	      if ((addr>>ck0)&0x1)
	           SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_MOSI);
	      else
	           CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_MOSI);
	  ...
	  // data Out
	  for (ck0=0;ck0<32;ck0++)
	  {
	      SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);
	      dataOut = (dataOut<<1)|READ_CBUS_REG_BITS(PREG_PAD_GPIO3_I, IO_MISO, 1);
	      CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_SCK);
	      CPU_wait(1);        
	  }
	  
	  SET_CBUS_REG_MASK(PREG_PAD_GPIO0_O, 1<<IO_CSB);
	  return (dataOut);
}
	  	  
void apb_write_reg_serial_dvbt(int addr, int val)
{
    int ck0;
    gpio_reset();
    SET_CBUS_REG_MASK(PREG_PAD_GPIO3_O, 1<<IO_CSB);
...
int fpga_apb_read_reg(int reg)
{
	 apb_read_reg_serial_dvbt(reg);
	 CPU_wait(1000);
   return (apb_read_reg_serial_dvbt(reg));
}

void fpga_apb_write_reg(int reg,int val)
{
   apb_write_reg_serial_dvbt(reg,val);
   CPU_wait(1000);
}
...
static u32 demod_addr_chg(int mode, int addr)
{
    ...
    case 4 : tmp = ATSC_BASE + addr*4; break;
    default : tmp = addr;
    }
    return tmp;
}
...
{
#ifdef CONFIG_AM_DEMOD_FPGA_VER
   // printk("mode is %d\n", mode);
    if ((mode == 4)&& (data <= 0x3ff)) // For atsc mode
    {
    	   printk("data is %x\n",data);
    	   fpga_apb_write_reg(0,(addr&0xffff)<<8 | (data&0xff));
    }
    else{ 
    	   fpga_apb_write_reg(addr,data);   
		   printk("data is %x\n",data);
    	}
#else
    u32 tmp;
...
u32 apb_read_reg(int mode, int addr)
{
    ...
    if (mode == 4)
    {
    	  fpga_apb_write_reg(4,(addr&0xffff)<<8);
    	  ret = fpga_apb_read_reg(0)&0xff;
    }
    else    
        ret = fpga_apb_read_reg(addr);
#else
    tmp = demod_addr_chg(mode, addr);
...
{
	init_tuner_for_my_tool(tuner_sys);
}
