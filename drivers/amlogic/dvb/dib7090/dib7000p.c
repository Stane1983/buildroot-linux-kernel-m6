...
static int dib7000p_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *fep)
{
	...
	fe_status_t stat;
	dib7000p_read_status(fe, &stat);
	printk("fp>>: [stat:%d] [HP:%d, LP:%d, CO:%d, TR:%d, GU:%d, HI:%d]\n", 
		stat,
		fep->u.ofdm.code_rate_HP, fep->u.ofdm.code_rate_LP, 
		fep->u.ofdm.constellation, fep->u.ofdm.transmission_mode,
		fep->u.ofdm.guard_interval, fep->u.ofdm.hierarchy_information);
