...
int crypto_done(crypto_device_t * pcrypt_info)
{
    ...
    key = (char *)kmalloc(pcrypt_info->keysize,GFP_KERNEL);
    if(key==NULL)
    {
        return ret;
    }
    copy_from_user((void *)key,(void*)pcrypt_info->keyaddr,pcrypt_info->keysize);
    ...
    ret = crypto_blkcipher_setkey(tfm, (unsigned char *)key, pcrypt_info->keysize);
    if (ret) {
