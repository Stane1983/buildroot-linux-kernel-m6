...
static int i2c_gsl_ts_suspend(struct i2c_client *client, pm_message_t state)
{
	struct gsl_ts *ts = dev_get_drvdata(&(client->dev));
	int rc = 0;
	
	printk("I'am in i2c_gsl_ts_suspend() start\n");
	
#ifdef GSL_TIMER
	printk( "i2c_gsl_ts_suspend () : delete gsl_timer\n");
...
static int i2c_gsl_ts_resume(struct i2c_client *client)
{
	struct gsl_ts *ts = dev_get_drvdata(&(client->dev));
	int rc = 0;

	printk("I'am in i2c_gsl_ts_resume() start\n");
	ts->is_suspended = false;
	if (ts->pdata->resume) 
	{
		rc = ts->pdata->resume();
...
#ifdef GSL_TIMER
	printk( "i2c_gsl_ts_resume () : add gsl_timer\n");
	...
	enable_irq(ts->irq);	 
	printk("I'am in i2c_gsl_ts_resume() end\n");
	return 0;
...
//#ifndef CONFIG_HAS_EARLYSUSPEND
	.suspend	= i2c_gsl_ts_suspend,
	.resume	= i2c_gsl_ts_resume,
//#endif
	.probe		= gsl_ts_probe,
