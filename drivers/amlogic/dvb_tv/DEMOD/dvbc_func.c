...
static u32 dvbc_get_per(void)
{
    ...
    u32 acc_rs_per_times = apb_read_reg(QAM_BASE+0xcc)&0xffff;
...
//	apb_write_reg(DEMOD_CFG_BASE,0x00000007);
    ...
    //apb_write_reg(QAM_BASE+0x020, 0x21353e54);  // PHS_reset & TIM_CTRO_ACCURATE  sw_tim_select=0
	  ...
	  apb_write_reg(QAM_BASE+0x020, 0x61b53e54); //modified by qiancheng by raymond 20121208
    ...
    apb_write_reg(QAM_BASE+0x038, 0x00200000);  // TIM_SWEEP_RANGE 16000
...
//    demod_sts->dat0 = tuner_get_ch_power(demod_i2c);
 //   demod_sts->dat1 = tuner_get_ch_power(demod_i2c);
    ...
    tmp = demod_sts->dat1;
    printk("strength %ddb  0xe0 status is %d ,b4 status is %d", tmp,(
	apb_read_reg(QAM_BASE+0xe0)&0xffff),(apb_read_reg(QAM_BASE+0xb4)&0xffff));
	printk("dagc_gain is %d ", apb_read_reg(QAM_BASE+0xa4)&0x7f);
