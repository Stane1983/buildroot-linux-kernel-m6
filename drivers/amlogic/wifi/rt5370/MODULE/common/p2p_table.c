...
#ifdef WFD_SUPPORT
        /* WFD */
        Tab->Client[i].WfdEntryInfo.bWfdClient = 0;
        Tab->Client[i].WfdEntryInfo.wfd_devive_type = 0;
        Tab->Client[i].WfdEntryInfo.source_coupled = 0;
        Tab->Client[i].WfdEntryInfo.sink_coupled = 0;
        Tab->Client[i].WfdEntryInfo.session_avail = 0;
        Tab->Client[i].WfdEntryInfo.wfd_service_discovery = 0;
        Tab->Client[i].WfdEntryInfo.wfd_PC = 0;
        Tab->Client[i].WfdEntryInfo.wfd_CP = 0;
        Tab->Client[i].WfdEntryInfo.wfd_time_sync = 0;
        Tab->Client[i].WfdEntryInfo.rtsp_port = 0;
        Tab->Client[i].WfdEntryInfo.max_throughput = 0;
#endif /* WFD_SUPPORT */
	}
	Tab->ClientNumber = 0;
	return;
	...
	switch (MsgType)
	{
...
#ifdef WFD_SUPPORT
			p2p_dev_info.bWfdClient = pP2pEntry->WfdEntryInfo.bWfdClient;
			if (pP2pEntry->WfdEntryInfo.bWfdClient)
			{
				p2p_dev_info.wfd_devive_type = pP2pEntry->WfdEntryInfo.wfd_devive_type;
				p2p_dev_info.source_coupled = pP2pEntry->WfdEntryInfo.source_coupled;
				p2p_dev_info.session_avail = pP2pEntry->WfdEntryInfo.session_avail;
				p2p_dev_info.sink_coupled = pP2pEntry->WfdEntryInfo.sink_coupled;
				p2p_dev_info.wfd_service_discovery = pP2pEntry->WfdEntryInfo.wfd_service_discovery;
				p2p_dev_info.wfd_PC = pP2pEntry->WfdEntryInfo.wfd_PC;
				p2p_dev_info.wfd_CP = pP2pEntry->WfdEntryInfo.wfd_CP;
				p2p_dev_info.wfd_time_sync = pP2pEntry->WfdEntryInfo.wfd_time_sync;
				p2p_dev_info.rtsp_port = pP2pEntry->WfdEntryInfo.rtsp_port;
				p2p_dev_info.max_throughput = pP2pEntry->WfdEntryInfo.max_throughput;
				RTMPMoveMemory(p2p_dev_info.assoc_addr, pP2pEntry->WfdEntryInfo.assoc_addr, MAC_ADDR_LEN);
				p2p_dev_info.coupled_sink_status.CoupledStat = pP2pEntry->WfdEntryInfo.coupled_sink_status.CoupledStat;
			}
#endif /* WFD_SUPPORT */
		...
		case RT_P2P_CONNECTED:
			...
			if (p2pIdx < MAX_P2P_GROUP_SIZE)
				p2p_group_info.rtsp_port = pAd->P2pTable.Client[p2pIdx].WfdEntryInfo.rtsp_port;
			DBGPRINT(RT_DEBUG_TRACE, ("%s::RTSP Port: %d\n", __FUNCTION__, p2p_group_info.rtsp_port));
#endif /* WFD_SUPPORT */
