...
printk("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz\n");
    ...
    if (bInitial)
    {
        ...
        DBGPRINT(RT_DEBUG_TRACE, ("%s:: set bInitial!!! pBssEntry->AvgRssiX8 = %d, pBssEntry->AvgRssi = %d, pBssEntry->Rssi= %d\n", __func__, pBssEntry->AvgRssiX8, pBssEntry->AvgRssi, pBssEntry->Rssi));
    }
    else
    {
        pBssEntry->AvgRssiX8 = (pBssEntry->AvgRssiX8 - pBssEntry->AvgRssi) + pBssEntry->Rssi;
        DBGPRINT(RT_DEBUG_TRACE, ("%s:: pBssEntry->AvgRssiX8 = %d, pBssEntry->AvgRssi = %d, pBssEntry->Rssi= %d\n", __func__, pBssEntry->AvgRssiX8, pBssEntry->AvgRssi, pBssEntry->Rssi));
    }
    ...
    pSignal->Rssi = (__u8)(pBssEntry->AvgRssi)+6;
	...
	IN	ULONG					Data)
{
	...
	for(IdBss=0; IdBss<pAd->ScanTab.BssNr; IdBss++)
	{
		...
		printk("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n");
	}
	...
	NdisAllocateSpinLock(pAd, &pAd->StaCtIf.Lock);
		...
		case CMD_RTPRIV_IOCTL_BSS_LIST_GET:
		{
			...
			for (i = 0; i <pBssList->MaxNum ; i++)
			{
				...
				printk("yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy\n");
			}
		}
			break;
