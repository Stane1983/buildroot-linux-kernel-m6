...
static struct rt3261_init_reg init_list[] = {
	//{RT3261_DUMMY1		, 0x701},//fa[12:13] = 1'b; fa[8~10]=1; fa[0]=1
	{RT3261_ADDA_CLK1	, 0x0014},//73[2] = 1'b //shaoshuai 9-28 //bard 10-11
...
//	{RT3261_HP_CALIB_AMP_DET, 0x0420},
	{RT3261_SPK_L_MIXER	, 0x003c},//OUTMIXL -> SPKMIXL //bard 10-11
	{RT3261_SPK_R_MIXER	, 0x003c},//OUTMIXR -> SPKMIXR //bard 10-11
	{RT3261_SPK_VOL		, 0x8888},//SPKMIX -> SPKVOL //shaoshuai 10-10
	{RT3261_SPO_CLSD_RATIO	, 0x0001},
	{RT3261_SPO_L_MIXER	, 0xc800},//SPKVOLL/SPKVOLR -> SPOLMIX //bard 10-11
	{RT3261_SPO_R_MIXER	, 0x3800},//mute SPORMIX //bard 10-11
//	{RT3261_SPO_L_MIXER	, 0xb800},//DAC -> SPOLMIX
//	{RT3261_SPO_R_MIXER	, 0x1800},//DAC -> SPORMIX  
//	{RT3261_I2S1_SDP	, 0xD000},//change IIS1 and IIS2
	/*record*/
	{RT3261_IN1_IN2		, 0x5080},//IN1 boost 40db and differential mode
...
//	{RT3261_REC_R2_MIXER	, 0x006f},//Mic2 -> RECMIXR
    ...
    {RT3261_I2S2_SDP   , 0x8002},   //shaoshuai 9-28
    {RT3261_GEN_CTRL1   , 0xF01},//shaoshuai 10-8
	{RT3261_CLS_D_OVCD,  0x0334},//shaoshuai 10-8 //bard 10-11
...
int rt3261_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if(jack_insert) {
		if (SND_SOC_BIAS_OFF == codec->dapm.bias_level) {
			...
			snd_soc_write(codec, RT3261_DUMMY1 , 0x3701);
		}
		...
		snd_soc_update_bits(codec, RT3261_DUMMY1,
			0x1, 0x1);
		msleep(100);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	case I2S2_MODE_3G:
		snd_soc_update_bits(codec, RT3261_PWR_DIG1, 
			RT3261_PWR_ADC_L | RT3261_PWR_ADC_R,
			RT3261_PWR_ADC_L | RT3261_PWR_ADC_R);
		snd_soc_update_bits(codec, RT3261_PWR_DIG2, 
			RT3261_PWR_ADC_MF_L | RT3261_PWR_ADC_MF_R,
			RT3261_PWR_ADC_MF_L | RT3261_PWR_ADC_MF_R);
		snd_soc_update_bits(codec, RT3261_PWR_ANLG1, 
			RT3261_PWR_LDO2, RT3261_PWR_LDO2);
		snd_soc_update_bits(codec, RT3261_PWR_ANLG2, 
			RT3261_PWR_BST1 | RT3261_PWR_BST4,
			RT3261_PWR_BST1 | RT3261_PWR_BST4);
		snd_soc_update_bits(codec, RT3261_PWR_MIXER, 
			RT3261_PWR_RM_L | RT3261_PWR_RM_R,
			RT3261_PWR_RM_L | RT3261_PWR_RM_R);
		snd_soc_update_bits(codec, RT3261_PWR_VOL, 
			RT3261_PWR_IN_L | RT3261_PWR_IN_R,
			RT3261_PWR_IN_L | RT3261_PWR_IN_R);
	...
	default:
		snd_soc_update_bits(codec, RT3261_PWR_DIG1, 
			RT3261_PWR_ADC_L | RT3261_PWR_ADC_R, 0);
		snd_soc_update_bits(codec, RT3261_PWR_DIG2, 
			RT3261_PWR_ADC_MF_L | RT3261_PWR_ADC_MF_R, 0);
		snd_soc_update_bits(codec, RT3261_PWR_ANLG1, 
			RT3261_PWR_LDO2, 0);
		snd_soc_update_bits(codec, RT3261_PWR_ANLG2, 
			RT3261_PWR_BST1 | RT3261_PWR_BST4, 0);
		snd_soc_update_bits(codec, RT3261_PWR_MIXER, 
			RT3261_PWR_RM_L | RT3261_PWR_RM_R, 0);
		snd_soc_update_bits(codec, RT3261_PWR_VOL, 
			RT3261_PWR_IN_L | RT3261_PWR_IN_R, 0);
		rt3261_index_update_bits(codec,
...
static const struct snd_soc_dapm_widget rt3261_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("HPOR MIX", SND_SOC_NOPM, 0, 0,
		rt3261_hpo_mix, ARRAY_SIZE(rt3261_hpo_mix)),
	SND_SOC_DAPM_MIXER("LOUT MIX", RT3261_PWR_ANLG1, RT3261_PWR_LM_BIT, 0,
	...
	SND_SOC_DAPM_PGA_S("HP amp", 2, SND_SOC_NOPM, 0, 0,
...
static const struct snd_soc_dapm_route rt3261_dapm_routes[] = {
	...
	{"ADC L", NULL, "RECMIXL"},
	{"ADC L", NULL, "ADC L power"}, //bard 10-11
	{"ADC R", NULL, "RECMIXR"},
	{"ADC R", NULL, "ADC R power"}, //bard 10-11
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	printk("rt3261->sysclk=%d rt3261->lrck[%d]=%d\n",rt3261->sysclk,dai->id,rt3261->lrck[dai->id]); //bard 9-27
	if (pre_div < 0) {
	...
	dai_sel |= (RT3261_U_IF1 | RT3261_U_IF2); //bard 9-27
	if (dai_sel < 0) {
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		...
		snd_soc_update_bits(codec, RT3261_SPK_VOL,
			RT3261_L_MUTE | RT3261_R_MUTE,
			RT3261_L_MUTE | RT3261_R_MUTE); //bard 10-11
		snd_soc_update_bits(codec, RT3261_PWR_ANLG2,
	...
	case SND_SOC_BIAS_OFF:
		...
		snd_soc_write(codec, RT3261_DUMMY1, 0x3700);
		    ...
		    const struct i2c_device_id *id)
{
	...
	int ret;
    printk("rt3261, re3261_i2c_probe\n");
